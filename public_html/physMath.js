 ...
 */
  ...
  function getSurfacesAtSoonestAngleTime(aState) {
    var startAngle = aState.a;
    console.groupCollapsed("getSurfacesAtSoonestAngleTime");
  ...
  function solveEarliestSurfaceEndpoint(state, surface) {
    ...
    //var velNorm = state.vel.normalize();
    var v01 = surface.p1.subtract(surface.p0);
    
    var xVecSign = (v01.x < 0 ? -1 : 1);
    var yVecSign = (v01.y < 0 ? -1 : 1);

    var xSign = (state.vel.x < 0 ? -1 : 1);
    var ySign = (state.vel.y < 0 ? -1 : 1);


    var time0;
    var time1;

    if (xVecSign === xSign && yVecSign === ySign) {                   // moving away from p0
      time0 = solveTimeToDist1D(results.distanceP0, results.parallelVel, results.parallelAccel, 0.005);
      time1 = solveTimeToDist1D(results.distanceP1, results.parallelVel, results.parallelAccel, 0);
    } else {                                                          // moving away from p1
      time0 = solveTimeToDist1D(results.distanceP0, results.parallelVel, results.parallelAccel, 0);
      time1 = solveTimeToDist1D(results.distanceP1, results.parallelVel, results.parallelAccel, 0.005);
    }
  ...
  function getNextSurfaceData(state, surface) {
    ...
    /* result = {   concave: t/f, 
                    convex: t/f, 
                    badConcavity: t/f, 
                    angle: angle extending from one normal to the other, or smallest angle in case of undefined concavity
                    movingTowards0: 
                    movingTowards1: }   */
    state.print("");
    var concRes0 = getLineLineConcavity(surface, surface.adjacent0, state);
    var concRes1 = getLineLineConcavity(surface, surface.adjacent1, state);

    console.log("concRes0: ", concRes0);
    console.log("concRes1: ", concRes1);

    if (concRes0 && concRes0.concave) {
      if (concRes0.movingTowards1 || getDistFromLine(state.pos, surface.adjacent0) > state.radius + 0.00001) {    // intentionally 1, its because surface.adjacent0 is passed in as line1.
        console.log("getDistFromLine(state.pos, surface.adjacent0) " + getDistFromLine(state.pos, surface.adjacent0));
        time0 = solveTimeToDistFromLine(state.pos, state.vel, state.accel, surface.adjacent0, state.radius);
      } else {
        console.log("too close to line 0, ignoring for 0.0005???");
        time0 = solveTimeToDistFromLine(state.pos, state.vel, state.accel, surface.adjacent0, state.radius, 0.0005);
      }
    }
    if (concRes1 && concRes1.concave) {
      if (concRes1.movingTowards1 || getDistFromLine(state.pos, surface.adjacent1) > state.radius + 0.00001) {
        console.log("getDistFromLine(state.pos, surface.adjacent1) " + getDistFromLine(state.pos, surface.adjacent1));
        time1 = solveTimeToDistFromLine(state.pos, state.vel, state.accel, surface.adjacent1, state.radius);
      } else {
        console.log("too close to line 1, ignoring for 0.0005???");
        time1 = solveTimeToDistFromLine(state.pos, state.vel, state.accel, surface.adjacent1, state.radius, 0.0005);
      }
    }
    ...
    if (closestPos === undefined || closestPos === null) {
      ...
      console.log("time0 was closestPositive: ", closestPos);
      console.log("concRes0: ", concRes0);
      data = { adjNumber: 0, time: state.time + time0, angle: concRes0.relativeAngle };
    } else if (closestPos === time1) {
      console.log("time1 was closestPositive: ", closestPos);
      console.log("concRes1: ", concRes1);
      data = { adjNumber: 1, time: state.time + time1, angle: concRes1.relativeAngle };
    } else {
      throw "what the balls man, closest positive isnt time0, time1, or null???";
    }
    console.groupEnd();
   ...
   */
  function solveTimeToDist1D(targetDist, currentVelocity, acceleration, minTime) {
    console.groupCollapsed("solving time to dist 1D,");
    console.log("solveTimeToDist1D.  acceleration ", acceleration, ", currentVelocity ", currentVelocity, ", distanceToSurfaceEnd ", targetDist, ", minTime ", minTime);
    ...
    if (acceleration === 0) {
      ...
      if ((minTime || minTime === 0) && time <= minTime) {
        console.groupEnd();
        return time;
      }
      ...
      if (x < 0) {
        // ROOTS ARE IMAGINARY!
        console.groupEnd("timeToDist1D: roots are imaginary, not gonna exit surface.");
        return time;
      } else {
        //calculate roots
        //console.log("x: ", x);
        ...
        time = closestPositive(y, z, minTime);
      }
    }
    console.groupEnd();
  ...
  function getLineCollisionTime(state, line) {
    console.groupCollapsed("getLineCollisionTime()");
   ...
   */
  function solveTimeToDistFromLine(curPos, curVel, accel, targetLine, distanceGoal, minTime) {
    if (!minTime) {
      minTime = 0;
    }
    ...
    var time = solveTimeToDist1D(distance, results.perpVel, results.perpAccel, minTime);
  ...
  function closestPositive(value1, value2, minTime) {
    var toReturn = null;
    ...
    if (value1 <= (minTime || 0)) {            // is value1 negative?     //hackey bullshit to stop infinite looping???? TODO ????
      if (value2 <= (minTime || 0)) {
    ...
    } else if (value2 <= (minTime || 0)) {     // is value2 negative? we know value1 is positive.
  ...
  function getLineLineConcavity(line0, line1, referenceState) {
    //console.group("start getLineLineConcavity");
    var referencePos = referenceState.pos;
    var vec0;
    var vec1;
    var intersect;
    ...
    if (pointsEqual(line0.p0, line1.p1)) {
      ...
      console.log("wrong points equal????");
    ...
    var normTestLine0 = new L(vec0n.x, vec0n.y, vec0n.x + line0.normal.x, vec0n.y + line0.normal.y);
    var normTestLine1 = new L(vec1n.x, vec1n.y, vec1n.x + line1.normal.x, vec1n.y + line1.normal.y);
    var normTestIntersect = getLineLineIntersect(normTestLine0, normTestLine1);

    //console.log("normTestLine0 ", normTestLine0);
    //console.log("normTestLine1 ", normTestLine1);
    ...
    toReturn.relativeAngle = Math.acos(vec0n.dot(vec1n));
    var velAngleTo0 = Math.acos(line0.normal.dot(referenceState.vel.normalize()));
    var velAngleTo1 = Math.acos(line1.normal.dot(referenceState.vel.normalize()));

    toReturn.movingTowards0 = (velAngleTo0 > HALF_PI ? true : false);
    toReturn.movingTowards1 = (velAngleTo1 > HALF_PI ? true : false);
    //console.log("vec0n", vec0n);
    //console.log("vec1n", vec1n);
    //console.log("vec0n.dot(vec1n)", vec0n.dot(vec1n));
    //console.log("toReturn.angle", toReturn.angle);
    if (toReturn.convex) {
      toReturn.absoluteAngle = TWO_PI - toReturn.relativeAngle;
    } else {
      toReturn.absoluteAngle = toReturn.relativeAngle;
    }
    //console.groupEnd();
    /* var result = { concave: t/f, 
                      convex: t/f, 
                      badConcavity: t/f, 
                      angle: angle extending from one normal to the other, or smallest angle in case of undefined concavity
                      movingTowards0: 
  ...
  function getLineLineIntersect(line0, line1) {
    ...
    //g("getLineLineIntersect, heavy debug stuff");
    //var v0 = line0.p0.subtract(line0.p1);
    //var m0 = v0.y / v0.x;
    //l("v0.x = " + v0.x + ", v0.y = " + v0.y);       //if y or x are pretty much 0, we got problems? maybe?
    ...
    //var m1 = v1.y / v1.x;
    //l("v1.x = " + v1.x + ", v1.y = " + v1.y);       //if y or x are pretty much 0, we got problems? maybe?
    ...
    var v0 = line0.p0.subtract(line0.p1);
    //l("v0.x = " + v0.x + ", v0.y = " + v0.y);
    var m0;
    var b0;   
    if (v0.x !== 0) {                 // handle verticals....
      m0 = v0.y / v0.x;
      b0 = line0.p0.y - m0 * line0.p0.x;    // y = mx + b
      //l("b0 intercept0 = " + b0);
    } else {
      x = line0.p0.x;
      //l("line0 was vertical, set x to " + x);
    }
    ...
    if (v1.x !== 0) {                 // handle verticals
      m1 = v1.y / v1.x;
      b1 = line1.p0.y - m1 * line1.p0.x;    // y = mx + b
      //l("b1 intercept1 = " + b1);
    } else if (!x) {
      x = line1.p0.x;
      //l("line1 was vertical, set x to " + x);
    } else {
      l("line1 and line0 were both vertical, not sure what to do here....");
      //e();
      throw "both lines vertical. no intersect. Perhaps just return NaN vec?";
    }

    // m0*x + b0 = m1*x + b1      equation to find convergent x point
    if (!x) {                       // if no vertical x is undefined, define it normally.
      x = (b1 - b0) / (m0 - m1);
      //l("x = " + x);
    }
    if (m0) {                       // use m0 and b0 to fetch y intersect.

      //l("y = " + m0 * x + b0);
      //e();
      return new vec2(x, m0 * x + b0);
    } else {                        // else if line0 was vertical, use m1 and b1.
      //l("y = " + m1 * x + b1);
      //e();
      return new vec2(x, m1 * x + b1);
    }
  }
